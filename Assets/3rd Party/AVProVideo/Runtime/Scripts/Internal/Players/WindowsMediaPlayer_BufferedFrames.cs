						...
						_frameSelectionMode == BufferedFrameSelectionMode.ElapsedTimeVsynced)
				{
					...
					if (needsInitialFrame || playingBufferedFrames)
					{
						...
						else
						{
							...
							// use our own clock...
							double videoFrameDuration = 10000000.0 / (double)GetVideoFrameRate();
							long videoDuration = (long)Math.Floor(10000000.0 * GetDuration());
							long lastFrameTime = Math.Max(videoDuration, state.maxTimeStamp);
							double delta = 10000000.0 * Time.deltaTime;

							if (_frameSelectionMode == BufferedFrameSelectionMode.ElapsedTimeVsynced && QualitySettings.vSyncCount > 0)
							{
								// Since we're running with vsync enabled, the MINIMUM elapsed time will be 1 monitor refresh
								double minMonitorDuration = (QualitySettings.vSyncCount * 10000000.0) / (double)Screen.currentResolution.refreshRate;

								delta = minMonitorDuration;
